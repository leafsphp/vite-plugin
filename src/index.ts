        ...
        return (
            composer.packages?.find(
                (composerPackage: { name: string }) =>
                    composerPackage.name === 'leafs/leaf'
            )?.version ?? ''
        );
    } catch {
        return '';
...
function resolvePluginConfig(
    ...
    if (typeof config.publicDirectory === 'string') {
        ...
        if (config.publicDirectory === '') {
            throw new Error(
                "leaf-vite-plugin: publicDirectory must be a subdirectory. E.g. 'public'."
            );
        }
    }
    ...
    if (typeof config.buildDirectory === 'string') {
        ...
        if (config.buildDirectory === '') {
            throw new Error(
                "leaf-vite-plugin: buildDirectory must be a subdirectory. E.g. 'build'."
            );
        }
    }
    ...
    return {
        ...
        hotFile:
            config.hotFile ??
            path.join(config.publicDirectory ?? 'public', 'hot'),
...
function resolveDevServerUrl(
    ...
    const sailHost =
        process.env.LARAVEL_SAIL && !userConfig.server?.host
            ? 'localhost'
            : null;
...
function resolveDevelopmentEnvironmentServerConfig(
    ...
    if (!fs.existsSync(keyPath) || !fs.existsSync(certPath)) {
        ...
        if (
            configPath === herdMacConfigPath() ||
            configPath === herdWindowsConfigPath()
        ) {
            throw Error(
